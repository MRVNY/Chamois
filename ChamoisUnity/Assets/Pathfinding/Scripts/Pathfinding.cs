...
public class Pathfinding {
    ...
    public Pathfinding(int width, int height) {
        Instance = this;
        grid = new Grid<PathNode>(width, height, 1f, new Vector3(0,-600,0), (Grid<PathNode> g, int x, int y) => new PathNode(g, x, y));
        ...
        //         var nodePos = GetGrid().GetWorldPosition(i, j);
    ...
    public List<Vector3> FindPath(Vector3 startWorldPosition, Vector3 endWorldPosition) {
        ...
        if (path == null) {
            ...
            {
                //Vector3 tmp = new Vector3(pathNode.x, pathNode.y) * grid.GetCellSize() +
                 //             Vector3.one * grid.GetCellSize() * .5f;
                Vector3 tmp = grid.GetWorldPosition(pathNode.x, pathNode.y);
                vectorPath.Add(tmp);
            }
            return vectorPath;
        }
